import { TickerItem, tickerDataHashtag } from '@/data/tickerData';
    ...
    >
      <div className="flex w-full px-[0.5625] whitespace-nowrap overflow-hidden landscape:overflow-hidden">
        <div className="flex animate-scroll landscape:gap-[1.75rem]">
          {[...tickerDataHashtag, ...tickerDataHashtag].map((item: TickerItem) => (
            <div
              key={`${item.id}-${Math.random()}`}
              className="flex gap-[0.944] landscape:gap-[3.5rem] items-center mx-auto landscape:mx-auto h-full"
            >
              <span
                className="flex items-center text-white text-[0.32236rem] landscape:text-[1.2rem] font-bold uppercase
                         text-sm landscape:tracking-[0.096] underline"
              >
                {item.text}
              </span>
