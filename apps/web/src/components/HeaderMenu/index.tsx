...
const HeaderMenu: FC<HeaderMenuProps> = ({ links, isHorizontal, isInModal }) => {
    ...
    const classMenu = isHorizontal ? "flex font-bold gap-3 flex-row content-center justify-end items-center" :
                                              "flex font-bold gap-3 flex-col";
    const classNav = isHorizontal ? "text-accent uppercase font-bold flex h-full justify-end" :
                                    "text-accent uppercase font-bold flex h-full justify-start";
    const mainClass = isInModal ? "main-navigation font-sharp font-bold uppercase" :
